...
from errors.custom_exceptions import ValidationError, RepositoryError, IDError, ServiceError, UndoRedoError
import datetime
...
class Console:
    def __init__(self, movie_service, client_service, rental_service):
        ...
        self.__commands = {'am': self.ui_add_movie, 'ac': self.ui_add_client, 'rm': self.ui_remove_movie,
                           'rc': self.ui_remove_client,
                           'um': self.ui_update_movie, 'uc': self.ui_update_client, 'lm': self.ui_list_movies,
                           'lc': self.ui_list_clients,
                           'rem': self.ui_rent_movie, 'rtm': self.ui_return_movie, 'sm': self.ui_search_movies,
                           'sc': self.ui_search_clients,
                           'statm': self.ui_most_rented_movies_statistics,
                           'statc': self.ui_most_active_clients_statistics,
    ...
    def ui_remove_movie(self):
        ...
        UndoManager.register_operation(self.__movie_service, UndoHandlerEnum.REMOVE_MOVIE, removed_movie,
    ...
    def ui_update_movie(self):
        ...
        messages = {'t': Color.OKGREEN + 'Please input a new movie title: ' + Color.ENDC,
                    'd': Color.OKGREEN + 'Please input a new movie description: ' + Color.ENDC,
                    'g': Color.OKGREEN + 'Please input a new movie genre: ' + Color.ENDC}

        print(
            Color.OKGREEN + ' t - Update movie title\n d - Update movie description\n g - Update movie genre' + Color.ENDC)
        command = input('>> ').strip()
